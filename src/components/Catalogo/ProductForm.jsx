...
export default function ProductForm({ onClose, prefill }) {
  ...
  const [precioM,    setPrecioM]    = useState(prefill?.precio ? String(prefill.precio) : '')
  const [precioMay,  setPrecioMay]  = useState('')
  const [costo,      setCosto]      = useState(prefill?.costo ? String(prefill.costo) : '')
  const [stock,      setStock]      = useState('0')
  const [stockB,     setStockB]     = useState('0')
  const [minMayoreo, setMinMayoreo] = useState('')
  ...
  function save() {
    ...
    if (tipo === 'pinza' && !subtipo.trim()) { toast('Ingresa el tipo de pinza','er'); return }
    if (!hasFiber && tipo !== 'pinza' && !nombre.trim()) { toast('Ingresa el nombre','er'); return }
    ...
    const base = {
      ...
      minMayoreo:    parseInt(minMayoreo) || 0,
    ...
    // Check duplicates
    const exists = (db.prods || []).filter(p => {
      if (p.tipo !== tipo) return false
      if (tipo === 'clasica') {
        return (p.marca||'Nagaraku') === (marca||'Nagaraku') &&
          p.grosor === grosor &&
          p.curva === (curva==='custom'?'custom':curva) &&
          (p.notas||'') === notas.trim() &&
          newProds.some(n => n.largo === p.largo)
      }
      if (tipo === 'tec') {
        return p.marca === marca &&
          p.dimension === (dimension==='custom'?'custom':dimension) &&
          p.curva === (curva==='custom'?'custom':curva) &&
          (p.notas||'') === notas.trim() &&
          newProds.some(n => n.largo === p.largo)
      }
      ...
      if (tipo === 'adhesivo') return p.marca === marca && p.nombre === nombre.trim()
      if (tipo === 'pinza') return p.marca === marca && p.subtipo === (subtipo.trim()||nombre.trim())
      return p.nombre === nombre.trim()
    })

    if (exists.length > 0) {
      const names = exists.map(p => {
        if (hasFiber) return (p.largo==='Mix'?'Mix':(p.largoLabel||p.largo)+'mm')
        return p.nombre || p.subtipo
      }).join(', ')
      const go = confirm(`Ya existe este producto con largo(s): ${names}\n\n¿Agregar de todas formas?`)
      if (!go) return
    }
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="card space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <p className="text-xs text-ink-2 mb-1">Min. piezas mayoreo</p>
                <input type="number" inputMode="numeric" className="input" placeholder="ej. 3"
                  value={minMayoreo} onChange={e=>setMinMayoreo(e.target.value)}/>
              </div>
            </div>
          </div>

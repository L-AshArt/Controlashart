...
export default function ProductForm({ onClose }) {
  ...
  const [largos,     setLargos]     = useState([])   // multi-select
  ...
  function save() {
    ...
    if ((tipo==='clasica'||tipo==='tec'||tipo==='abanico') && largos.length===0) { toast('Selecciona al menos un largo', 'er'); return }
    ...
    // Create one product per largo selected
    const largoList = (tipo==='clasica'||tipo==='tec'||tipo==='abanico') ? largos : ['']
    const newProds = largoList.map((l, i) => ({
      ...prod,
      id: Date.now() + i,
    ...
    <div className="fixed inset-0 z-40 bg-black/50 flex items-end">
      <div className="bg-white w-full rounded-t-2xl max-h-[92vh] overflow-y-auto">
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          {(tipo==='clasica'||tipo==='tec'||tipo==='abanico') && (
            <div className="card">
              <label className="section-title">Largo <span className="text-rosa font-bold">({largos.length} seleccionados)</span></label>
              <div className="flex gap-1.5 flex-wrap mt-1">
                {LARGOS.map(l => (
                  <button key={l} onClick={()=>toggleLargo(l)}
                    className={`chip text-xs ${largos.includes(l)?'chip-active':''}`}>
                    {l==='Mix'?'Mix':l+'mm'}
                  </button>
                ))}
                <button onClick={()=>toggleLargo('custom')}
                  className={`chip text-xs ${largos.includes('custom')?'chip-active':''}`}>
                  Otro
                </button>
              </div>
              {largos.includes('custom') && <input className="input mt-2" placeholder="ej. 19mm" value={largoLabel} onChange={e=>setLargoLabel(e.target.value)}/>}
              {largos.length > 1 && (
                <p className="text-xs text-ok mt-2">Se crearán {largos.length} productos con el mismo precio</p>
              )}
            </div>
          )}

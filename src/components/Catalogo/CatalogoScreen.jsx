...
import { pNom, tLabel, tColor, grosorP, marcaP, numSort, stockColor } from '../../utils/products'
...
export default function CatalogoScreen() {
  ...
  const grouped = useMemo(() => {
    ...
    return Object.keys(groups).sort((a,b)=>a==='Sin marca'?1:b==='Sin marca'?-1:a.localeCompare(b))
  ...
  return (
    <div>
      <Header
        ...
        right={
          isAdmin && (
            <button onClick={()=>setShowForm(true)} className="btn-primary px-3 py-2 text-sm flex items-center gap-1.5">
                    ...
                    <div key={p.id} className="card flex items-center gap-3">
                      ...
                      {isAdmin && (
                        <div className="flex flex-col gap-1.5">
                          ...
                          <button onClick={()=>delProd(p.id)} className="w-8 h-8 rounded-lg bg-danger-light text-danger flex items-center justify-center">
                            <Trash2 size={14}/>
                          </button>
                        </div>
                      )}

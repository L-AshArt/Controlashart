...
export default function TicketModal({ venta, onClose }) {
  ...
  const url = `${window.location.origin}${window.location.pathname}ticket.html?v=${encodeURIComponent(JSON.stringify(venta))}`
  ...
  async function sharePDF() {
    ...
    try {
      if (typeof html2canvas !== 'undefined' && typeof window.jspdf !== 'undefined') {
        ...
        document.body.removeChild(wrap)
        alert('Cargando... intenta de nuevo')
      }
    } catch (e) {
    ...
    <div className="fixed inset-0 z-40 bg-black/50 flex items-end">
      <div className="bg-white w-full rounded-t-2xl p-5 space-y-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="card flex flex-col items-center gap-3">
          <p className="text-xs font-bold text-ink-3 uppercase tracking-wider">Código QR del ticket</p>
          <div ref={qrRef} className="rounded-xl overflow-hidden"/>
          <p className="text-xs text-ink-3 text-center">Escanea para ver el ticket digital</p>
        </div>
...
function buildTicketHTML(v) {
  ...
  return `<div style="background:#FAF0F2;padding:14px;font-family:Arial,sans-serif;width:560px">
    <div style="background:#fff;border-radius:16px;padding:20px;margin-bottom:10px;text-align:center">
      ...
      <div style="display:flex;justify-content:space-between;padding:8px 0 4px;font-size:17px;font-weight:700;color:#1C1C2E">
        <span>Total</span><span>$${v.tot.toFixed(2)}</span></div>
    ...
    <div style="background:#fff;border-radius:16px;padding:14px;margin-bottom:10px">
      <div style="display:flex;gap:8px">
        <div style="flex:1;display:flex;align-items:center;justify-content:center;background:#25D366;color:#fff;font-weight:700;font-size:13px;padding:12px;border-radius:12px">WhatsApp</div>
        <div style="flex:1;display:flex;align-items:center;justify-content:center;background:#C4798A;color:#fff;font-weight:700;font-size:13px;padding:12px;border-radius:12px">@lash_art_store</div>
      </div>
    </div>
  </div>`
}

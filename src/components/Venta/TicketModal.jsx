...
export default function TicketModal({ venta, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/50 flex items-end overflow-y-auto">
      <div className="bg-white w-full rounded-t-2xl max-h-[95vh] overflow-y-auto">
        ...
        {/* PDF preview — este div ES lo que se captura */}
        <div ref={pdfRef} style={{background:'#FAF0F2', padding:'14px', fontFamily:'Arial,sans-serif'}}>
          {/* Ticket card */}
          <div style={{background:'#fff', borderRadius:'16px', padding:'20px', marginBottom:'10px', textAlign:'center'}}>
            <img src={LOGO} alt="logo"
              style={{width:'70px', height:'70px', borderRadius:'50%', objectFit:'cover', margin:'0 auto 10px', display:'block'}}/>
            <div style={{fontSize:'22px', fontWeight:'700', color:'#1C1C2E'}}>L-Ash Art</div>
            <div style={{fontSize:'12px', color:'#6B6B7B', marginTop:'3px'}}>Gracias por tu compra!</div>
            <div style={{fontSize:'10px', color:'#9B9BAB', marginTop:'2px'}}>Venta #{venta.num} - {venta.fecha} {venta.hora}</div>
            <div style={{borderTop:'1px dashed #EAEAF0', margin:'12px 0'}}/>
            {venta.items?.map((it,i) => (
              <div key={i} style={{display:'flex', justifyContent:'space-between', padding:'6px 0', borderBottom:'1px solid #F5F5FA'}}>
                <div style={{flex:1, marginRight:'8px', textAlign:'left'}}>
                  <div style={{fontSize:'12px', fontWeight:'600', color:'#1C1C2E'}}>{it.nombre}</div>
                  <div style={{fontSize:'11px', color:'#9B9BAB'}}>{it.qty} ud. x ${it.precio?.toFixed(2)}</div>
                </div>
                <div style={{fontSize:'12px', fontWeight:'700', color:'#1C1C2E', whiteSpace:'nowrap'}}>${(it.qty*it.precio).toFixed(2)}</div>
              </div>
            ))}
            <div style={{borderTop:'1px dashed #EAEAF0', margin:'10px 0'}}/>
            <div style={{display:'flex', justifyContent:'space-between', padding:'8px 0', fontSize:'17px', fontWeight:'700', color:'#1C1C2E'}}>
              <span>Total</span><span>${venta.tot?.toFixed(2)}</span>
            </div>
            <div style={{marginTop:'8px', textAlign:'left'}}>
              <span style={{background:'#FAF0F2', color:'#C4798A', fontSize:'10px', fontWeight:'700', padding:'3px 10px', borderRadius:'20px'}}>{m}</span>
            </div>
          </div>
          ...
          <div style={{background:'#fff', borderRadius:'16px', padding:'14px', marginBottom:'10px'}}>
            <div style={{display:'flex', gap:'8px'}}>
              <div style={{flex:1, display:'flex', alignItems:'center', justifyContent:'center', background:'#25D366', color:'#fff', fontWeight:'700', fontSize:'13px', padding:'12px', borderRadius:'12px'}}>WhatsApp</div>
              <div style={{flex:1, display:'flex', alignItems:'center', justifyContent:'center', background:'#C4798A', color:'#fff', fontWeight:'700', fontSize:'13px', padding:'12px', borderRadius:'12px'}}>@lash_art_store</div>
            </div>
          </div>

          {/* Catalogo */}
          <div style={{background:'#fff', borderRadius:'16px', padding:'14px'}}>
            <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', background:'#FAF0F2', borderRadius:'10px', padding:'10px 12px'}}>
              <div>
                <div style={{fontSize:'13px', fontWeight:'700', color:'#1C1C2E'}}>Ver catalogo</div>
                <div style={{fontSize:'10px', color:'#6B6B7B'}}>lashart.kyte.site</div>
              </div>
              <div style={{fontSize:'20px'}}>&#128722;</div>
            </div>
          </div>
          {/* Maps button */}
          <a href="https://maps.app.goo.gl/ZBwxJ18qeHzyDxBH7" target="_blank" rel="noreferrer"
            style={{display:'flex',alignItems:'center',gap:'10px',background:'#E8F0FE',borderRadius:'12px',padding:'12px 14px',textDecoration:'none',marginTop:'8px'}}>
            <div style={{fontSize:'22px'}}>&#128205;</div>
            <div>
              <div style={{fontSize:'13px',fontWeight:'700',color:'#1A1A2E'}}>Como llegar</div>
              <div style={{fontSize:'10px',color:'#4285F4'}}>Ver en Google Maps</div>
            </div>
          </a>
        </div>
      </div>
      ...
      {/* QR */}
        <div className="px-5 py-4 flex flex-col items-center gap-3 border-t border-gray-100">

...
export default function TicketModal({ venta, onClose }) {
  ...
  const [status,  setStatus]  = useState('Preparando PDF...')
  ...
  // ── Pre-generar PDF cuando se abre el modal ──────────────────
  useEffect(() => {
    const iframe = document.createElement('iframe')
    // z-index:1 → detrás del modal (z-40), pero el browser lo renderiza
    iframe.style.cssText = 'position:fixed;left:0;top:0;width:480px;height:4000px;border:none;z-index:1;pointer-events:none'
    iframe.src = ticketURL + '&noauto=1'
    document.body.appendChild(iframe)
    iframeRef.current = iframe

    const fallbackTimer = setTimeout(() => {
      // Si tarda demasiado, habilitar botón con flujo de pestaña nueva
      setStatus('listo-fallback')
    }, 20000)

    iframe.onload = () => {
      // Esperar CDN scripts (jspdf + html2canvas) dentro del iframe
      setTimeout(async () => {
        try {
          const buildPDF = iframe.contentWindow?.buildPDF
          if (!buildPDF) throw new Error('buildPDF no disponible')
          const doc = await buildPDF()
          if (!doc) throw new Error('doc vacío')
          const blob = doc.output('blob')
          setPdfBlob(blob)
          setStatus('listo')
        } catch(e) {
          console.error('pre-gen error:', e)
          setStatus('listo-fallback')
        }
  ...
  // ── Compartir ─────────────────────────────────────────────────
  function sharePDF() {
    if (status === 'listo-fallback' || !pdfBlob) {
      // Fallback: abrir pestaña nueva con ?share=1
      window.open(ticketURL + '&share=1', '_blank')
      return
    }
    // PDF listo → compartir directo (menú de Android sin nueva pestaña)
    const file = new File([pdfBlob], 'ticket-' + venta.num + '.pdf', { type: 'application/pdf' })
    if (navigator.share && navigator.canShare({ files: [file] })) {
      navigator.share({ files: [file], title: 'Ticket L-Ash Art #' + venta.num })
  ...
  const ready   = status === 'listo' || status === 'listo-fallback'
  const btnLabel = !ready ? 'Preparando PDF...' : 'Compartir PDF'
    ...
    <div className="fixed inset-0 z-40 bg-black/50 flex items-end">
      <div className="bg-white w-full rounded-t-2xl" style={{maxHeight:'92vh',overflowY:'auto'}}>
        ...
        <div className="px-5 pb-6 pt-2 space-y-2">
          ...
          <button onClick={sharePDF} disabled={!ready}
            className="btn-primary w-full flex items-center justify-center gap-2 disabled:opacity-50">
            <Share2 size={18}/> {btnLabel}
          </button>

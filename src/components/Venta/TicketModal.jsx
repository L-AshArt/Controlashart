...
export default function TicketModal({ venta, onClose }) {
  ...
  async function sharePDF() {
    ...
    try {
      // Cargar ticket.html en iframe oculto (mismo origen)
      const iframe = document.createElement('iframe')
      iframe.src = ticketURL + '&noauto=1'
      iframe.style.cssText = 'position:fixed;left:0;top:0;width:480px;height:900px;border:none;opacity:0;pointer-events:none;z-index:-1'
      ...
      if (!getCanvas) throw new Error('getTicketCanvas no disponible')

      const rawCanvas = await getCanvas()
      document.body.removeChild(iframe)

      if (!rawCanvas) throw new Error('canvas vacío')

      // ── FIX FONDO NEGRO ──────────────────────────────────────────────
      // html2canvas puede devolver canvas con alpha transparente.
      // JPEG no soporta transparencia → negro. Componemos sobre fondo rosa.
      const bgCanvas = document.createElement('canvas')
      bgCanvas.width  = rawCanvas.width
      bgCanvas.height = rawCanvas.height
      const ctx = bgCanvas.getContext('2d')
      ctx.fillStyle = '#FAF0F2'
      ...
      const imgData = bgCanvas.toDataURL('image/jpeg', 0.92)
      const pw = 210
      const totalH = bgCanvas.height * pw / bgCanvas.width

...
export default function TicketModal({ venta, onClose }) {
  ...
  async function sharePDF() {
    setSharing(true)
    try {
      // Load ticket.html in hidden iframe (same origin = same content as scanned QR)
      ...
      const el = iDoc.getElementById('content')
      ...
      if (!h2c) throw new Error('html2canvas not loaded in iframe')
      ...
      // Generate PDF with links (same as ticket.html does)
      const jspdf = iframe.contentWindow.jspdf || window.jspdf
      const J = jspdf.jsPDF
      ...
      <div className="bg-white w-full rounded-t-2xl" style={{maxHeight:'92vh', overflowY:'auto'}}>
        ...
        <div className="px-5 pb-6 pt-2 space-y-2">
          ...
          <button onClick={sharePDF} disabled={sharing}
            ...
            <Share2 size={18}/> {sharing ? 'Generando PDF...' : 'Compartir PDF'}
          </button>

...
import { X, Banknote, CreditCard, Building2, CheckCircle, Split } from 'lucide-react'
...
const METHODS = [
  ...
  { id: 'dividido',      label: 'Dividido',      Icon: Split      },
]
...
export default function PaymentModal({ items, extraData, onClose, onDone }) {
  ...
  const [monto1, setMonto1]       = useState('')   // pago dividido parte 1
  ...
  function confirm() {
    ...
    const venta = {
      ...
      items: items.map(({prod,qty})=>({
        ...
        nombre: pNom(prod), precio: prod.precioMenudeo||0, qty,
        costo: prod.costo||0,
      })),
      tot: total, desc: disc, cambio,
      metodo: method==='dividido'?`${metodo1}+${metodo2}`:method,
      cliente: extraData?.clientName||'',
      nota: extraData?.note||'',
      cupon: extraData?.coupon?.code||'',
    }
    ...
    setDone(true)
    setTimeout(()=>{ onDone(); toast('Venta #'+num+' registrada','ok') }, 1200)
  }

  if (done) return (
    <div className="fixed inset-0 z-30 bg-ok flex flex-col items-center justify-center gap-4">
      <CheckCircle size={80} className="text-white" strokeWidth={1.5}/>
      <p className="text-white text-2xl font-bold">¡Venta registrada!</p>
      <p className="text-white/80 text-4xl font-bold">${total.toFixed(2)}</p>
    </div>
  )
    ...
    <div className="fixed inset-0 z-30 flex flex-col bg-surface-bg">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        <div className="card text-center py-6">
          ...
          {disc>0 && <p className="text-xs text-ok mt-1">Descuento aplicado: -${disc.toFixed(2)}</p>}
        </div>
        ...
        {method==='efectivo' && (
          <div className="card space-y-3">
            <label className="section-title">¿Con cuánto paga?</label>
        ...
        {method==='dividido' && (
          <div className="card space-y-3">
            <p className="section-title">Dividir pago</p>
            <div className="space-y-2">
              <div className="flex gap-2 items-center">
                <select className="input flex-1 text-sm" value={metodo1} onChange={e=>setMetodo1(e.target.value)}>
                  <option value="efectivo">Efectivo</option>
                  <option value="tarjeta">Tarjeta</option>
                  <option value="transferencia">Transferencia</option>
                </select>
                <input type="number" inputMode="decimal" className="input flex-1 text-center font-bold"
                  placeholder="$0.00" value={monto1} onChange={e=>setMonto1(e.target.value)}/>
              </div>
              <div className="flex gap-2 items-center">
                <select className="input flex-1 text-sm" value={metodo2} onChange={e=>setMetodo2(e.target.value)}>
                  <option value="efectivo">Efectivo</option>
                  <option value="tarjeta">Tarjeta</option>
                  <option value="transferencia">Transferencia</option>
                </select>
                <div className="input flex-1 text-center font-bold text-ink-2 flex items-center justify-center">
                  ${m2.toFixed(2)}
                </div>
              </div>
            </div>
            <div className="flex justify-between text-sm text-ink-2 pt-1 border-t border-gray-100">
              <span>Total</span>

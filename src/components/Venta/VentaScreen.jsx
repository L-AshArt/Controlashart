...
import { ShoppingCart, Star, Plus, Search, X } from 'lucide-react'
...
export default function VentaScreen() {
  ...
  const totalLibres = (libres||[]).reduce((s,l) => s + l.precio*l.qty, 0)
  const total    = cartTotal(db.prods || []) + totalLibres
  const countLibres = (libres||[]).reduce((s,l) => s + l.qty, 0)

  // Group prods by tipo for 'todos' view
    ...
    <div className="pb-24">
      ...
      <div className="px-4 pt-3 space-y-2">
        ...
        <div className="flex gap-2">
          ...
          {query && <button onClick={()=>setQuery('')} className="p-2 text-ink-3"><X size={18}/></button>}
        </div>
        ...
        {query ? (
          ...
          <div className="space-y-2">
            {prods.map((p,i) => (
              <ProductRow key={(p.id||i)+'_s_'+i} prod={p} qty={cart[p.id]||0}
                onAdd={addToCart} onRem={remFromCart}
                isFav={isFav(p.id)} onToggleFav={()=>toggle(p.id)}
                onEdit={isAdmin?setEditProd:null}/>
            ))}
            {prods.length === 0 && <p className="text-center text-ink-3 text-sm py-4">Sin resultados</p>}
          </div>
        ) : (
          // Grouped by tipo
          <div>
            {byTipo.map(([t, ps]) => (
              <div key={t} className="mb-4">
                ...
                {t === 'clasica'
                  ...
                  : <div className="space-y-2">
                      {ps.map((p,i) => (
                        <ProductRow key={(p.id||i)+'_'+i} prod={p} qty={cart[p.id]||0}
                          onAdd={addToCart} onRem={remFromCart}
                          isFav={isFav(p.id)} onToggleFav={()=>toggle(p.id)}
                          onEdit={isAdmin?setEditProd:null}/>
                      ))}
      ...
      {items.length > 0 && (
        <button onClick={()=>setShowCart(true)}
          ...
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full bg-rosa flex items-center justify-center text-xs font-bold">{cartCount + countLibres}</div>
      ...
      {showLibre && <LibreModal onClose={()=>setShowLibre(false)} onAddLibre={(item)=>{addLibre(item);setShowLibre(false)}} isAdmin={isAdmin}/>}
      {showCart  && (
        <Cart items={items} libres={libres} onAdd={addToCart} onRemove={remFromCart}
          onAddLibre={addLibre} onRemLibre={remLibre}
          onClose={()=>setShowCart(false)} onCheckout={handleCheckout}
          onClear={()=>{clearCart();setShowCart(false)}}
          isAdmin={isAdmin} db={db}/>
      )}
      {showPay && (
        <PaymentModal data={showPay} onClose={()=>setShowPay(null)}/>
      )}
    </div>
  )
}
      ...
      {Object.entries(dimGroups).sort(([a],[b])=>numSort(a,b)).map(([dim, gps]) => {
        ...
        return (
          <div key={gk} className="mb-2 pl-2">
            ...
            {prods2.map((prod, pi) => {
              const ss2 = prod.stock <= 0
              const qty2 = cart[prod.id]||0
              return (
                <div key={prod.id+'_'+pi} className="card flex items-center gap-2 mt-1">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-ink leading-tight">{pNom(prod)}</p>
                    <p className="text-xs text-ink-3 mt-0.5">
                      {ss2 ? 'Sin stock' : `$${prod.precioMenudeo?.toFixed(2)}`}
                  ...
                  <div className="flex items-center gap-1">
                    {qty2 > 0 && <>
                      ...
                      <span className="w-5 text-center font-bold text-sm">{qty2}</span>
                    </>}
                    <button onClick={()=>toggleFav(prod.id)} className="text-ink-3 active:text-gold p-1">
                    ...
                    <button onClick={()=>onAdd(prod)} disabled={ss2 || qty2 >= prod.stock}
                      className="w-8 h-8 rounded-lg bg-rosa text-white flex items-center justify-center disabled:opacity-40">
                      <Plus size={16}/>
                    </button>
                  </div>

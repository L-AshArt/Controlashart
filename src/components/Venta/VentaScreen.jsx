...
export default function VentaScreen() {
  ...
  const { cart, cartItems, addToCart, remFromCart, clearCart, cartCount, cartTotal, priceOverrides } = useCart()
  ...
  return (
    <div className="pb-24">
      ...
      {showLibre && <LibreModal onClose={()=>setShowLibre(false)} onAdd={(item)=>{addToCart(item);setShowLibre(false)}}/>}
      {showCart  && (
        <Cart items={items} onAdd={addToCart} onRemove={remFromCart}

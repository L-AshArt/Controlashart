  ...
  return (
    <div>
      ...
      <div className="px-4 pb-4 mt-2 space-y-2">
        {!query && (() => {
          const byTipo = {}
          prods.forEach(p=>{ if(!byTipo[p.tipo])byTipo[p.tipo]=[]; byTipo[p.tipo].push(p) })
          return Object.entries(byTipo).map(([t,ps])=>(
            <div key={t}>
              <div className="flex items-center gap-2 mb-2 mt-3">
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${tColor(t)}`}>{tLabel(t)}</span>
              </div>
              {t==='clasica'  ? <ClasicaSection  ps={ps} cart={cart} onAdd={addToCart} onRem={remFromCart} sel={selState} setSel={setSelState} isFav={isFav} toggleFav={toggle}/> :
               t==='tec'      ? <TecSection      ps={ps} cart={cart} onAdd={addToCart} onRem={remFromCart} sel={selState} setSel={setSelState} isFav={isFav} toggleFav={toggle}/> :
               t==='abanico'  ? <AbanicoSection  ps={ps} cart={cart} onAdd={addToCart} onRem={remFromCart} sel={selState} setSel={setSelState} isFav={isFav} toggleFav={toggle}/> :
               ps.map(p=><ProductRow key={p.id} prod={p} qty={cart[p.id]||0} onAdd={addToCart} onRem={remFromCart} isFav={isFav(p.id)} onToggleFav={()=>toggle(p.id)}/>)}
            </div>
          ))
        })()}
        {query && prods.map(p=>(
          <ProductRow key={p.id} prod={p} qty={cart[p.id]||0} onAdd={addToCart} onRem={remFromCart} isFav={isFav(p.id)} onToggleFav={()=>toggle(p.id)}/>
        ))}
      </div>
...
function ProductRow({ prod, qty, onAdd, onRem, isFav, onToggleFav }) {
...
function ClasicaSection({ ps, cart, onAdd, onRem, sel, setSel, isFav, toggleFav }) {
  const byMarca = {}
        ...
        return (
          <div key={grosor} className="mb-2">
            ...
            {st.curva&&st.largo&&pgC.filter(p=>p.largo===st.largo).map(prod=>(
              <ProductRow key={prod.id} prod={prod} qty={cart[prod.id]||0} onAdd={onAdd} onRem={onRem} isFav={isFav(prod.id)} onToggleFav={()=>toggleFav(prod.id)}/>
            ))}
          </div>
        )
      })}
...
function TecSection({ ps, cart, onAdd, onRem, sel, setSel, isFav, toggleFav }) {
  const groups = {}
  ps.forEach(p=>{ const d=p.dimension==='custom'?p.dimensionLabel||'Otro':p.dimension, k=`${p.marca}_${d}`; if(!groups[k])groups[k]={marca:p.marca,dim:d,ps:[]}; groups[k].ps.push(p) })
  return Object.entries(groups).sort(([,a],[,b])=>a.marca.localeCompare(b.marca)||numSort(a.dim,b.dim)).map(([gk,{marca,dim,ps:gps}])=>{
    ...
    return (
      <div key={gk} className="mb-2">
        <p className="text-xs font-semibold text-ink-2 mb-1.5 pl-1">{marca} · {dim}</p>
        <div className="flex gap-1.5 flex-wrap mb-1.5">
          <span className="text-[10px] text-ink-3 self-center">Curva</span>
          {cvs.map(c=>{
...
function AbanicoSection({ ps, cart, onAdd, onRem, sel, setSel, isFav, toggleFav }) {
  const byDim = {}

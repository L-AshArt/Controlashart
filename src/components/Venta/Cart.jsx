...
export default function Cart({ items, libres=[], onAdd, onRemove, onAddLibre, onRemLibre, onClose, onCheckout, onClear, isAdmin, db }) {
  const { priceOverrides, setOverride } = useCart()
  ...
  const libresTotal = (libres||[]).reduce((s,l) => s + l.precio*l.qty, 0)
  const subtotal = items.reduce((s, {prod, qty}) => s + getPrice(prod) * qty, 0) + libresTotal
  ...
  function confirmEditPrice(prod) {
    ...
    if (!isNaN(val) && val >= 0) {
      setOverride(prod.id, val)
    }
    setEditingPrice(null)
  }

  function checkout() {
    // Merge regular items + libres for payment
    const libreItems = (libres||[]).map(l => ({
      prod: { id: l.id, nombre: l.nombre, precioMenudeo: l.precio, costo: l.costo||0, tipo: 'otro', libre: true },
      qty: l.qty
    }))
    onCheckout({ items: [...items, ...libreItems], total, disc, clientName, coupon: couponApplied, note })
  }
              ...
              <div className="flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  <div className="marquee-wrap">
                    <span className={`text-sm font-semibold text-ink ${pNom(prod).length > 28 ? 'marquee-text' : 'marquee-text-static'}`}>
                      {pNom(prod)}
                    </span>
                  </div>
                    ...
                    {isEditing ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <input type="number" inputMode="decimal"
                          ...
                          value={tempPrice} onChange={e=>setTempPrice(e.target.value)} autoFocus/>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {prod._mayoreo && !hasOverride && (
                          <span className="text-[10px] font-bold text-gold bg-gold-light px-1.5 py-0.5 rounded-full">
                            mayoreo
                          </span>
                        )}
                        <button onClick={()=>startEditPrice(prod)} className="p-0.5 text-ink-3 active:text-rosa">
                          <Pencil size={12}/>
                        </button>
                      </div>
                    )}
          ...
          <div key={l.id} className="card">
            <div className="flex items-start gap-3">
              <div className="flex-1 min-w-0">
                ...
                <span className="text-[10px] font-bold text-rosa bg-rosa/10 px-1.5 py-0.5 rounded-full">Libre</span>
              </div>
              <div className="flex items-center gap-1.5">
                <button onClick={()=>onRemLibre&&onRemLibre(l.id)} className="w-7 h-7 rounded-lg bg-surface-bg text-ink font-bold flex items-center justify-center">−</button>
                <span className="w-5 text-center font-bold text-sm">{l.qty}</span>
                <button onClick={()=>onAddLibre&&onAddLibre(l)} className="w-7 h-7 rounded-lg bg-rosa text-white flex items-center justify-center">+</button>
                <span className="text-sm font-bold text-ink w-16 text-right">${(l.precio*l.qty).toFixed(2)}</span>
              </div>
            </div>
          </div>
        ))}

...
export function CartProvider({ children }) {
  ...
  const cartItems = (prods) => {
    ...
    raw.forEach(({ prod, qty }) => {
      const cat = (prod.tipo || 'otro') + '_' + (prod.marca || '')
    ...
    return raw.map(({ prod, qty }) => {
      ...
      // Mayoreo applies if category total >= minMayoreo (and product has mayoreo price)
      const autoMay = !override && prod.minMayoreo > 0 && prod.precioMayoreo > 0 && totalCat >= prod.minMayoreo
